...
const Register:React.FC = () => {
 ...
 const [formData,setFormData] = useState<FormDataPropsSignup>({username: '', email:'',password: ''});
 ...
 const handleGuestSignUp = async() => {
   const resultAction = await dispatch(login({email: 'guest@blogHorizon.com', password: '123456'}));
    ...
    return(
        <div className="w-full xl:w-1/2 flex flex-col justify-center px-40 gap-5">
          ...
          <form onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
               {/*name*/}
               <label id="name" className="flex items-center font-semibold"><CgAsterisk className="text-red-500"/> Name:</label>
               <PrimaryInputField name="name" type="text" className="border-2 border-[#d9d9d9] indent-5 py-1.5 rounded-lg 
                hover:border-blue-300 focus:outline-blue-500" icon={<FiUser/>} onChange={(e)=> setFormData({...formData,username:e.target.value})}/> 
               ...
               <InputPassword name="pass" className="border-2 border-[#d9d9d9] indent-5 py-1.5 rounded-lg 
                hover:border-blue-300 focus:outline-blue-500" value={showPass} onChange={(e)=> {setShowPass(e.target.value);  setFormData({...formData,password:e.target.value})}} 
                />

               {/*sumbit*/}
               <PrimaryButton  type="submit" name="Register" className="!rounded-xl mt-2 !bg-blue-800 hover:!bg-blue-700"/>
                  <span className="text-xl font-semibold text-center">0R</span>
               <PrimaryButton name="Register As A Guest"  onClick={handleGuestSignUp}
               className="!rounded-xl mt-2 !bg-blue-800 hover:!bg-blue-700"/>
                  <span className="text-xl font-semibold text-center">0R</span>
                 <GoogleAuthButton/>
          </form>
          <div className="text-md font-semibold">
             <span> Or </span> <Link to="/auth/login" className="text-blue-800 cursor-pointer">Already Have Account Login </Link>
          </div>
        </div>
    )
};
